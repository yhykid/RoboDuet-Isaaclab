...
def reward_lin_vel_z(
    ...
    duet_event: DuetEvent =  env.duet_manager.get_term(duet_name)
    terrain_names = duet_event.env_per_terrain_name
    asset: Articulation = env.scene[asset_cfg.name]
...
def reward_tracking_goal_vel(
    ...
    asset: Articulation = env.scene[asset_cfg.name]
    duet_event: DuetEvent = env.duet_manager.get_term(duet_name)
    target_pos_rel = duet_event.target_pos_rel
...
def reward_tracking_yaw(     
    ...
    yaw = torch.atan2(2*(q[:,0]*q[:,3] + q[:,1]*q[:,2]),
                    1 - 2*(q[:,2]**2 + q[:,3]**2))
    return torch.exp(-torch.abs((duet_event.target_yaw - yaw)))
