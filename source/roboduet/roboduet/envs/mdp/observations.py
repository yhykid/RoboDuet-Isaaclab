...
class RoboDuetObservations(ManagerTermBase): #todo 把friction 和restitution写到env里面去  然后在这读取
    def __init__(self, cfg: ObservationTermCfg, env: DuetManagerBasedRLEnv):
        ...
        self.clock_inputs = torch.zeros(self.num_envs, 4, dtype=torch.float, device=self.device,
                                        requires_grad=False)
        self.obs_buf = torch.zeros(self.num_envs, self.env.num_obs, device=self.device, dtype=torch.float)
        self.privileged_obs_buf = torch.zeros(self.num_envs, self.env.num_privileged_obs, device=self.device,
                                              dtype=torch.float)
...
class ArmObservations(ManagerTermBase): #todo 把friction 和restitution写到env里面去  然后在这读取
    def __init__(self, cfg: ObservationTermCfg, env: DuetManagerBasedRLEnv):
        super().__init__(cfg, env)
        self.asset: Articulation = env.scene[cfg.params["asset_cfg"].name]
        self.asset_cfg = cfg.params["asset_cfg"]
    ...
    def __call__(
        ...
        self.obs_buf = torch.cat(((self.asset.data.joint_pos[:,self.env.num_actions_loco:self.env.num_actions_loco+self.env.num_actions_arm] - 
                                   self.asset.data.default_joint_pos[:,self.env.num_actions_loco:self.env.num_actions_loco+self.env.num_actions_arm]) * obs_scales.dof_pos, #6
                                    env.action_manager.get_term('joint_pos')._arm_raw_actions[:,:6], # 6 
                                    commands_arm if global_switch.switch_open else torch.zeros_like(commands_arm[:]),
                                    roll.unsqueeze(1), #1
                                    pitch.unsqueeze(1),
...
class DogObservations(ManagerTermBase): #todo 把friction 和restitution写到env里面去  然后在这读取
    def __init__(self, cfg: ObservationTermCfg, env: DuetManagerBasedRLEnv):
        super().__init__(cfg, env)
        self.asset: Articulation = env.scene[cfg.params["asset_cfg"].name]
        self.asset_cfg = cfg.params["asset_cfg"]
    ...
    def __call__(
        ...
        self.obs_buf = torch.cat((self.asset.data.projected_gravity_b,
                            (self.asset.data.joint_pos[:,:self.env.num_actions_loco] - self.asset.data.default_joint_pos[:,:self.env.num_actions_loco]) * obs_scales.dof_pos, #12
                            self.asset.data.joint_vel[:,:self.env.num_actions_loco] * obs_scales.dof_vel, # 12
                            env.action_manager.get_term('joint_pos')._leg_raw_actions, #12
                            (commands_dog*torch.tensor([obs_scales.lin_vel, obs_scales.lin_vel, obs_scales.ang_vel, obs_scales.body_pitch_cmd, obs_scales.body_roll_cmd], 
...
class Dog_PrivObservations(ManagerTermBase): 
    def __init__(self, cfg: ObservationTermCfg, env: DuetManagerBasedRLEnv):
        super().__init__(cfg, env)
        self.asset: Articulation = env.scene[cfg.params["asset_cfg"].name]
        self.asset_cfg = cfg.params["asset_cfg"]
    ...
    def __call__(
        ...
        privileged_obs_buf = torch.cat([torch.zeros((self.num_envs, 2), device=self.device)], dim=-1)
        return privileged_obs_buf


    

class Arm_PrivObservations(ManagerTermBase): 
    def __init__(self, cfg: ObservationTermCfg, env: DuetManagerBasedRLEnv):
        super().__init__(cfg, env)
        self.asset: Articulation = env.scene[cfg.params["asset_cfg"].name]
        self.asset_cfg = cfg.params["asset_cfg"]
    ...
    def __call__(
        ...
        #                                (self.friction_coeffs[:, 0].unsqueeze(1) - friction_coeffs_shift) * friction_coeffs_scale, dim=1)
        privileged_obs_buf = torch.cat([torch.zeros((self.num_envs, 2), device=self.device)], dim=-1)
        self.grip_id = self.asset.find_bodies(['zarx_body6'])[0][0]
        self.end_effector_state = self.asset.data.body_state_w[:,self.grip_id,:]
        lpy = self.get_lpy_in_base_coord(torch.arange(self.num_envs, device=self.device))
        forward = quat_apply(self.asset.data.root_quat_w, self.forward_vec)
    ...
    def get_lpy_in_base_coord(self, env_ids):
        forward = quat_apply(self.asset.data.root_quat_w[env_ids], self.forward_vec[env_ids])
        yaw = torch.atan2(forward[:, 1], forward[:, 0])
        ...
        x = torch.cos(yaw) * (self.grasper_in_world[:, 0] - self.asset.data.root_state_w[env_ids, 0]) \
            + torch.sin(yaw) * (self.grasper_in_world[:, 1] - self.asset.data.root_state_w[env_ids, 1])
        y = -torch.sin(yaw) * (self.grasper_in_world[:, 0] - self.asset.data.root_state_w[env_ids, 0]) \
            + torch.cos(yaw) * (self.grasper_in_world[:, 1] - self.asset.data.root_state_w[env_ids, 1])
        z = torch.mean(self.grasper_in_world[:, 2].unsqueeze(1), dim=1) - 0.38
