...
class PPODuet(PPO):
    ...
    def init_storage(self, training_type, num_envs, num_transitions_per_env, actor_obs_shape, privileged_obs_shape, obs_history_shape,
        ...
        self.storage = RolloutStorageDuet(training_type,
                                          num_envs, 
    ...
    def act(self, obs, privileged_obs, obs_history):
        ...
        self.transition.actions = self.policy.act(obs_history).detach()
        self.transition.values = self.policy.evaluate(obs_history, privileged_obs).detach()
    ...
    def update(self, un_adapt=False):
        ...
        for obs_batch, critic_obs_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch, \
            ...
            self.policy.act(obs_history_batch, masks=masks_batch)
            actions_log_prob_batch = self.policy.get_actions_log_prob(actions_batch)
            value_batch = self.policy.evaluate(obs_history_batch, privileged_obs_batch, masks=masks_batch)
        ...
        loss_dict = {
            ...
            "adaption_loss" : mean_adaptation_module_loss,
            "decoder_loss" : mean_decoder_loss,
            "decoder_loss_student" : mean_decoder_loss_student,
            "adaption_test_loss" : mean_adaptation_module_test_loss,
            "decoder_test_loss" : mean_decoder_test_loss,
            "decoder_test_loss_student" : mean_decoder_test_loss_student,
        }
        if self.rnd:
